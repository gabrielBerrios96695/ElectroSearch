...
<div class="container">
    ...
    <table class="table table-striped table-bordered">
        <thead class="thead-dark">
            <tr>
                ...
                <th>Ventas Completadas</th> <!-- Nueva columna -->
                <th>Total de Ventas</th> <!-- Nueva columna -->
                <th>Acciones</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $seller['completed_sales_count'] }}</td> <!-- Contador de ventas completadas -->
                <td>{{ number_format($seller['completed_sales_amount'], 2) }} Bs</td> <!-- Total de ventas completadas -->
                <td>
                    <button 
                        class="btn btn-info btn-sm" 
                        data-bs-toggle="modal" 
                        data-bs-target="#sellerModal{{ $loop->index }}"
                    >
                        Ver Productos
                    </button>
                </td>
    ...
    <h5 class="mt-4">Total Acumulado: {{ number_format(collect($sellersWithProducts)->sum('total_sales'), 2) }} Bs</h5>

    <!-- Modales para mostrar productos -->
    @foreach($sellersWithProducts as $index => $seller)
    <div class="modal fade" id="sellerModal{{ $index }}" tabindex="-1" aria-labelledby="sellerModalLabel{{ $index }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="sellerModalLabel{{ $index }}">Productos Vendidos por {{ $seller['seller_name'] }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    @if($seller['products']->isEmpty())
                        <p>No se encontraron productos vendidos por este vendedor.</p>
                    @else
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ number_format($product->total_sales, 2) }} Bs</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    <script>
        ...
        const sellersData = @json($sellersWithProducts);

        const labels = sellersData.map(seller => seller.seller_name);
